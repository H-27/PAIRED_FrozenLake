...
class DQN_Adversary():
    ...
    def __init__(self, alpha, gamma, epsilon, adversary_memory_size, adversary_batch_size, adversary_network, map_width, map_height, max_step=250):
        ...
        self.buffer = buffers.Direction_DQN_Buffer(adversary_memory_size, map_width, map_height)
    ...
    def collect_trajectories(self, adv_map, agent, agent_max_episodes):
        ...
        #for e in range(episodes):
            ...
            while not done:
                ...
                max_step_counter += 1
                if (win):
                    steps_per_episode_in_won.append(episode_steps)
            # get loss
            ...
            if (e != 0 and e % 500 == 0):
                ...
                wins = 0
            e += 1
            if (max_step_counter > self.max_step_size):
                max_step_reached = True
        win_ratio = 0 if wins == 0 else wins/e
        if(not steps_per_episode_in_won):
            steps_per_episode_in_won.append(100)
        return losses, win_ratio, shaped_cumulative_discounted_rewards, cumulative_discounted_rewards, steps_per_episode, steps_per_episode_in_won
